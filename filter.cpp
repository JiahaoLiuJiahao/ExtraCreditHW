...
void average(uchar* in, uchar* out, int size, int W, int H) {
	for(int i = 0; i < W; i++) {
		for(int j = 0; j < H; j++) {
			int miX = max(0, i - size), maX = min(W-1, i + size);
			int miY = max(0, j - size), maY = min(H-1, j + size);
			...
			for(int c = 0; c < 3; c++) {
				...
				for(int x = miX; x <= maX; x ++)
					for(int y = miY; y <= maY; y++)
						sum += in[(x + W*y) * 3 + c];
				out[(i + W*j) * 3 + c] = max(0.0, min(255.0, sum / d));
			}
		}
	}
}
			...
			for(int c = 0; c < 3; c++) {
				...
				for(int x = miX; x <= maX; x ++) {
					for(int y = miY; y <= maY; y++) {
						...
						double a = exp( - abs(colij - col) / sigma1 - (dx*dx + dy*dy) / sigma2);
						sum += col * a;
						w += a;
					}
				}
